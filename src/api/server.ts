...
app.post('/api/prompt', async (req: any, res: any) => {
    try {
        ...
        // Let the agent process the prompt and decide which skills to use
        console.log('📨 Sending prompt to agent...');
        
        // Since the user confirmed the agent skills work properly,
        // we'll use the agent's natural conversation ability
        // The agent should be able to handle free-form prompts
        let agentResponse;
        try {
            // Try different methods to interact with the agent
            if (typeof (BookAssistantAgent as any).prompt === 'function') {
                agentResponse = await (BookAssistantAgent as any).prompt(prompt);
            } else if (typeof (BookAssistantAgent as any).process === 'function') {
                agentResponse = await (BookAssistantAgent as any).process(prompt);
            } else {
                // Fallback: use the agent's chat capability and extract response
                const chatSession = await BookAssistantAgent.chat(prompt);
                agentResponse = chatSession;
            }
        } catch (error) {
        ...
        // Handle the response safely - extract just the text content
        let responseText = '';
        try {
            if (typeof agentResponse === 'string') {
                ...
                if (typeof (agentResponse as any).text === 'function') {
                    responseText = await (agentResponse as any).text();
                } else if (typeof (agentResponse as any).toString === 'function') {
                    responseText = (agentResponse as any).toString();
                } else {
                    // Try common response properties
                    responseText = (agentResponse as any).content || 
                                 (agentResponse as any).message || 
                                 (agentResponse as any).response ||
                                 'Agent response received but could not extract text';
                }
            } else {
            ...
            responseText = 'Error extracting response from agent';
        }
